...
from ._comet_common import (
    ...
    resolve_checkpoint, detect_hidden_sizes, detect_architecture,
    ARCH_REGRESSION, ARCH_REFERENCELESS, ARCH_UNIFIED,
)


class COMETRegressionModel(nn.Module):
    """Unified COMET scoring backbone supporting reference / referenceless / unified archs."""
    ...
    def __init__(self, pretrained: str = "xlm-roberta-large",
        ...
        n_feats = {ARCH_REGRESSION: 6, ARCH_REFERENCELESS: 4, ARCH_UNIFIED: 1}[architecture]
        self.estimator = Estimator(in_dim=hidden_dim * n_feats,
    ...
    def forward_separate(self, src_ids, src_mask, mt_ids, mt_mask,
        ...
        def sentemb(ids, mask):
            return avg_pool(self._encode_and_mix(ids, mask), mask)
...
class COMETScore:
    ...
    Auto-detects architecture (regression / referenceless / unified) from the
    checkpoint's FF input dim, so swapping `model_name` to any compatible
    Unbabel/* ckpt works without flag changes.
    """
    ...
    def __init__(self, qe: bool = False, model_name: str = None,
                 pretrained: str = None, hidden_sizes=None,
        ...
        # CometKiwi / unified models use infoxlm-large + softmax (the upstream
        # "sparsemax_patch" hparam silently falls through to softmax).
        ...
        ckpt_path = resolve_checkpoint(self._model_name, checkpoint_path)
        ckpt = torch.load(ckpt_path, map_location="cpu", weights_only=False)
        state_dict = ckpt.get("state_dict", ckpt)

        hidden_dim = AutoConfig.from_pretrained(pretrained).hidden_size
        self.architecture = detect_architecture(state_dict, hidden_dim)
        ...
        self.model = COMETRegressionModel(
            ...
            dropout=dropout, architecture=self.architecture,
